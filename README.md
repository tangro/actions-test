...
You can use a specific `version` of this action. The latest published version is `v1.1.5`. You can also use `latest` to always get the latest version.
...
test:
  ...
  steps:
    ...
    - name: Run npm install
      run: npm install
    - name: Run tests
      uses: tangro/actions-test@v1.1.5
      env:
